...
import com.difl.natesroommvvm.db.NoteDao;
...
public class ListFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        NoteDatabase db = Room.databaseBuilder(getContext(), NoteDatabase.class,
                "person-db").allowMainThreadQueries().build();

        List<Note> personList = db.noteDao().getAd();

        RecyclerView rv = view.findViewById(R.id.rv);
        rv.setLayoutManager(new GridLayoutManager(getContext(), 2));

        NewAdapter adapter = new NewAdapter(personList);
